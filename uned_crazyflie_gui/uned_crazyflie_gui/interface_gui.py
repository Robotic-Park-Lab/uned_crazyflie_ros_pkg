...
class RQT_Panel(QWidget):
    ...
    def add_external_window_widget(self):
        ...
        # self.window_id = self.wait_for_window_id(pid=self._process.get_pid(),
        # window_name=self._window_name, timeout=self._timeout_to_window_discovery)
    ...
    def wait_for_window_id(self):
        ...
        while window_id is None and (now - ini_t) < self._timeout_to_window_discovery:
            if pid is not None:
                ...
                raise RuntimeError(
                    "No PID or window_name provided to look for a window on wait_for_window_id")
            time.sleep(0.2)
            now = time.time()
        return window_id
...
if __name__ == '__main__':
    ...
    str_match = [s for s in output if "uned_cf_interface" in s]
    if not str_match:
        interface_node = rclpy.create_node('uned_cf_interface')
    else:
        node_name = "uned_cf_interface_aux" + str(len(str_match))
