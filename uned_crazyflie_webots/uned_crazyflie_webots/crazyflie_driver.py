...
class CrazyflieWebotsDriver_Test:
    def init(self, webots_node, properties):
        self.robot = webots_node.robot
        timestep = int(self.robot.getBasicTimeStep())
        ...
        rclpy.init(args=None)
        self.node = rclpy.create_node('crazyflie_webots_driver_test')
        self.node.create_subscription(Twist, '/dron02/cmd_vel', self.cmd_vel_callback, 1)
        self.laser_publisher = self.node.create_publisher(LaserScan, '/dron02/scan', 10)
        self.odom_publisher = self.node.create_publisher(Odometry, '/dron02/odom', 10)
