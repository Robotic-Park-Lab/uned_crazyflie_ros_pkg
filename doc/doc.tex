...
La transformación desde el centro de referencia del sistema global al dron está determinada por tres rotaciones sucesivas: Yaw $\left(\psi\right) \rightarrow$ Pitch $\left(\theta\right)\rightarrow$ Roll $\left(\phi\right)$. IMPORTANTE: Se emplea la representación extrínseca de Tait-Bryan. La matriz de rotación resultante sería: 
\begin{equation}
	Rot_{\phi} = \begin{pmatrix} 1 & 0 & 0\\ 0 & \cos\phi & \sin\phi\\	0 & -\sin\phi & \cos\phi\end{pmatrix}
	Rot_{\theta} = \begin{pmatrix} \cos\theta & 0 & -\sin\theta\\ 0 & 1 & 0\\\sin\theta & 0 & \cos\theta\end{pmatrix}
	Rot_{\psi} = \begin{pmatrix} \cos\psi & \sin\psi & 0\\ -\sin\psi & \cos\psi  & 0\\0 & 0 & 1 \end{pmatrix}
	\label{eq:Matriz-Rot}
\end{equation}
\begin{equation}
	R^{b}_{o} = 
...
\textcolor{red}{Las correcciones en rojo son las modificaciones que hay que hacerle a la matriz para que concuerde con la matriz que expone Barrientos \citep{barrientos2007fundamentos} en la ecuación 3.13 de acuerdo al orden de rotaciones que expone \cite{luis2016design}. En \citep{green2019autonomous} también aparecen los signos cambiados respecto a Barrientos, pero indica mal las matrices de rotación individuales. En \citep{greiff2017modelling} sí justifica la razón de los signos cambiados y expone también las matrices individuales correctas. Debe haber algún cambio de pi/2 por algún lado.}
